"""
agent_rag.py — Core AI agent logic with dynamic RAG context injection.
"""
...
from prompts.system_prompt_rag import build_system_prompt_rag
from retriever_rag import ClosiraRAGRetriever
from logger_rag import log_escalation, log_summary

MAX_RETRIES = 3
UNANSWERED_THRESHOLD = 2  # escalate after this many unanswered questions


class ClosiraRAGAgent:
    def __init__(self):
        ...
        self.retriever = ClosiraRAGRetriever()
    ...
    def chat(self, user_message: str) -> dict:
        ...
        retrieved_chunks, query_has_match = self.retriever.retrieve(user_message, current_stage=self.stage)

        # 2. Build RAG context
        if not query_has_match and self.stage == "answering_question":
            # Explicit out-of-SOP queries with zero matching keywords are immediately given empty context
            self.current_context = "[No matching information found in the Standard Operating Procedure.]"
        elif not retrieved_chunks:
            self.current_context = "[No matching information found in the Standard Operating Procedure.]"
        else:
            self.current_context = "\n\n---\n\n".join([c["text"] for c in retrieved_chunks])

        # 3. Append stage hint & stored memory
        ...
        # 5. Post-process and update state
    ...
    def _call_api(self, retry: int = 0) -> dict:
        ...
        try:
            # Build system prompt dynamically injecting the retrieved context
            system_prompt = build_system_prompt_rag(self.current_context)
    ...
    def generate_summary(self) -> str:
        ...
        # Inject the full SOP text for summary generation to ensure accuracy
